document.addEventListener('DOMContentLoaded', () => {
    ...
    const createArticleBook = book => {
        ...
        const pagesParagraphEl = document.createElement('p');
        yearParagraphEl.innerHTML = `Page : ${book.page}`;
        ...
        articleEl.append(pagesParagraphEl);
    ...
    const updateInfoBook = id => {
        ...
        if (index !== -1) {
            ...
            const newPage = parseInt(prompt(`Masukkan Halaman Baru( Halaman lama ${books[index].author})`));
            const emptyNewTitle = newTitle.length === 0;
            const emptyNewAuthor = newAuthor.length === 0;
            const invalidNewYear = typeof newYear !== 'number';
            const invalidNewPage = typeof newPage !== 'number';

            if (emptyNewTitle || emptyNewAuthor || invalidNewYear || invalidNewPage){
